...
def _merge_results(result_groups: list[list[dict]], top_k: int) -> list[dict]:
    seen: set[str] = set()
    merged: list[dict] = []
...
async def enhanced_query(request: EnhancedQueryRequest) -> EnhancedQueryResponse:
    try:
        ...
        retrieval_tasks = [
            engine.retrieve(query=subquery, top_k=request.top_k, partition=request.partition)
            for subquery in subqueries
        ]
        result_groups = await asyncio.gather(*retrieval_tasks)
        merged_results = _merge_results(result_groups, request.top_k)

        sub_answers: list[dict[str, Any]] = []
        if understanding["is_decomposed"]:
            answer_tasks = [
                generator.generate(query=subquery, context_docs=docs, use_cache=False)
                for subquery, docs in zip(subqueries, result_groups, strict=False)
            ]
            answers = await asyncio.gather(*answer_tasks)
            sub_answers = [
                {"query": subquery, "answer": answer, "result_count": len(docs)}
                for subquery, answer, docs in zip(subqueries, answers, result_groups, strict=False)
            ]
