...
async def init_vector_store() -> None:
    ...
    if backend == "postgres":
        from src.storage.postgres_store import init_postgres_store
        await init_postgres_store()
    else:
        from src.storage.vector_store import init_vector_store as init_chroma
        await init_chroma()
...
async def close_vector_store() -> None:
    ...
    backend = _get_backend()
    if backend == "postgres":
        from src.storage.postgres_store import close_postgres_store
        await close_postgres_store()
    else:
        from src.storage.vector_store import close_vector_store as close_chroma
        await close_chroma()
...
async def add_documents(
    ...
    backend = _get_backend()
    if backend == "postgres":
        from src.storage.postgres_store import add_documents as add_postgres
        return await add_postgres(ids, embeddings, documents, metadatas, partition)
    else:
        from src.storage.vector_store import add_documents as add_chroma
...
async def search(
    ...
    backend = _get_backend()
    if backend == "postgres":
        from src.storage.postgres_store import vector_search
        return await vector_search(query_embedding, top_k, partition)
    else:
        from src.storage.vector_store import search as search_chroma
...
async def hybrid_search(
    ...
    backend = _get_backend()
    if backend == "postgres":
        from src.storage.postgres_store import hybrid_search as hybrid_pg
...
async def delete_document(document_id: str) -> None:
    ...
    backend = _get_backend()
    if backend == "postgres":
        from src.storage.postgres_store import delete_document as delete_postgres
        await delete_postgres(document_id)
    else:
        from src.storage.vector_store import delete_document as delete_chroma
...
async def list_documents(skip: int = 0, limit: int = 100) -> list[dict]:
    ...
    backend = _get_backend()
    if backend == "postgres":
        from src.storage.postgres_store import list_documents as list_postgres
        return await list_postgres(skip, limit)
    else:
        from src.storage.vector_store import list_documents as list_chroma
